...
export class HUD {
  constructor() {
    this.el = {
      ...
      handbrakeIndicator: document.getElementById('handbrake-indicator'),
  ...
  updateDashboard(physics, input) {
    ...
    if (this.el.wheelSvg) {
      // Tỉ lệ vô lăng/bánh xe ~14:1, cùng công thức với mô hình vô lăng 3D trong vehicle.js
      const degrees = -physics.steerAngle * 14 * (180 / Math.PI);
      this.el.wheelSvg.style.transform = `rotate(${degrees}deg)`;
    }

    this.el.throttleBar.style.width = `${Math.round(input.throttle * 100)}%`;
    this.el.brakeBar.style.width = `${Math.round(input.brake * 100)}%`;
    if (this.el.handbrakeIndicator) {
      this.el.handbrakeIndicator.classList.toggle('on', !!input.handbrake);
    }
